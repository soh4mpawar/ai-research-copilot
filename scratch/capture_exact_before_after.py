...
focused_after_path = os.path.join(artifact_dir, "citation_graph_after_connected.png")
...
with sync_playwright() as p:
    ...
    # 4. Capture "AFTER" (Clean 54-node Connected Subgraph)
    print("4. Capturing AFTER (Connected Subgraph)...")
    page.screenshot(path=focused_after_path, full_page=False)
    print(f"[OK] Saved AFTER: {focused_after_path}")
    
    # 5. Capture "BEFORE" (Unchecking Connected Filter via Streamlit checkbox label click)
    print("5. Clicking checkbox label to uncheck and show 200-node scatter...")
    page.locator("[data-testid='stCheckbox'] label").first.click()
    time.sleep(6)
