...
with sync_playwright() as p:
    ...
    print("2. Activating Dark Mode Toggle...")
    try:
        # Find dark mode toggle in sidebar
        toggle = page.locator("label:has-text('Dark Mode')")
        if toggle.count() > 0:
            toggle.first.click()
            time.sleep(3)
        else:
            page.locator("[data-testid='stToggle']").first.click()
            time.sleep(3)
    except Exception as e:
        print("Error toggling dark mode:", e)
        
    # Run Research Pipeline on QA view
    print("3. Executing QA Query in Dark Mode...")
    try:
        btn = page.locator("button:has-text('Run Research Pipeline')")
        if btn.count() > 0:
            btn.first.click()
            # Wait for synthesis card
    ...
    print("4. Switching to Evaluation & RAGAS Dashboard in Dark Mode...")
    try:
    ...
    browser.close()
    print("Dark mode capture complete!")
