...
with sync_playwright() as p:
    ...
    print("3. Running QA Query and waiting for completion...")
    try:
        # Click Run Research Pipeline
        btn = page.locator("button:has-text('Run Research Pipeline')")
        if btn.count() > 0:
            btn.first.click()
            # Wait until spinner text disappears
            page.wait_for_selector(".badge-strong:has-text('Evidence: Strong')", timeout=35000)
            time.sleep(3)
